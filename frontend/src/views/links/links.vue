          ...
          <div v-else>
            ...
            <div v-for="link in links" class="link">
                ...
                <div v-show="wantUpdateLink.wantUpdate && wantUpdateLink.link_id === link.id">
                  <form @submit.prevent="onSubmitUpdateLink(link.id)">
                    <div class="grid gap-2 flex-col justify-center items-center">
                      <div class="link-content">
                        <Input id="name" type="text" v-model="form.name" class="placeholder:text-white"
                          placeholder="description of url" cy="edit-name" />
                        <Input id="url" type="text" v-model="form.url" class="placeholder:text-white" placeholder="url"
                          cy="edit-url" />
              ...
              <div class="link-icon">
                ...
                <i class="fa-regular fa-pen-to-square" @click="
                  wantUpdateLink.wantUpdate = opposite(wantUpdateLink.wantUpdate);
                wantUpdateLink.link_id = link.id;
                " cy="link-edit"></i>
              </div>
            </div>
          </div>
          <div v-show="wantAddLink" class="">
            <form @submit.prevent="onSubmitLink">
              <div class="grid gap-2 flex-col justify-center items-center link">
                <div class="link-content">
                  <Input id="name" type="text" v-model="form.name" class="placeholder:text-white"
                    placeholder="description of url" cy="add-name" />
                  <Input id="url" type="text" v-model="form.url" class="placeholder:text-white" placeholder="url"
                    cy="add-url" />
